...
$date2 = explode('/', $_POST['start-date']);
$start_date = jalali_to_gregorian($date2[0], $date2[1], $date2[2], TRUE);

$date2 = explode('/', $_POST['end-date']);
$end_date = jalali_to_gregorian($date2[0], $date2[1], $date2[2], TRUE);
...
if ($start_date != $end_date) {
    if (isset($_POST['search_box']) && $_POST['search_box'] != "") {
        $search_query .= " And (CAST(factorproducts.Date AS date) BETWEEN '$start_date' AND '$end_date')";
    } else {
        $search_query .= " WHERE CAST(factorproducts.Date AS date) BETWEEN '$start_date' AND '$end_date'";
    }
}
...
$factorProduct->open();
$factorProducts = $factorProduct->Search($search_query);
$factorProduct->close();
?>
