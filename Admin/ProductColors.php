...
نمیتوانید رنگ محصول جدید اضافه نمایید';
                        ...
                        <table class="footable table table-stripped" data-page-size="1000000000"
                            ...
                            foreach ($productcolors as $p) {
                                ...
                                echo "<td><div class='DatabaseField' >" . $p->Color->Name . "</div></td>";
                                echo "<td><div class='DatabaseField' >" . $p->Quantity . "</div></td>";
                                echo "<td><button class='btn-white btn btn-sm m-xs' title='Edit'><a href='ProductColor.php?";
                                if (isset($_GET['id']) == TRUE) {
                                    echo "pid=$p->Product&";
                                }
                                echo "id=$p->ProductColorId'>ویرایش</a></button></td>";
                                echo "<td><button class='btn-white btn btn-sm m-xs' title='Delete'><a onclick='return deleteConfirm()' href='operateProductColor.php?";
                                if (isset($_GET['id']) == TRUE) {
                                    echo "pid=$p->Product&";
                                }
                                echo "id=$p->ProductColorId'>حذف</a></button></td>";
                                echo "</tr>";
                            }
                            ?>
                            </tbody>
